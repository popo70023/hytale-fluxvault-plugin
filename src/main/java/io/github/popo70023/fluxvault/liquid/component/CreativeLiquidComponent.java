...
public class CreativeLiquidComponent implements Component<ChunkStore>, IFluxProvider {
    public static final String ID = "CreativeLiquidComponent";
    public static final BuilderCodec<CreativeLiquidComponent> CODEC = BuilderCodec.builder(CreativeLiquidComponent.class, CreativeLiquidComponent::new)
            .append(new KeyedCodec<>("LiquidId", Codec.STRING), (o, v) -> o.liquidID = v, (o) -> o.liquidID)
            .documentation("Set to a LiquidId to act as an Infinite Source (provides endless liquid). Leave empty (null) or set to '" + Liquid.EMPTY_ID + "' to act as a Liquid Void (destroys incoming liquid).").add()
            .build();

    private String liquidID;
    private final IFluxHandler<LiquidFlux> fluxHandler = new IFluxHandler<>() {
        @Override
        public boolean matchesFluxType(FluxType<?, ?> fluxType) {
            return LiquidFlux.class.isAssignableFrom(fluxType.getResourceClass());
        }
    ...
    public CreativeLiquidComponent(String liquidID) {
        this.liquidID = liquidID;
    }

    public static ComponentType<ChunkStore, CreativeLiquidComponent> getComponentType() {
        return ComponentTypes.CREATIVE_LIQUID_COMPONENT;
    }
    ...
    public Component<ChunkStore> clone() {
        return new CreativeLiquidComponent(liquidID);
    }
}
